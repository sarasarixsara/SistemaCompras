...
$startRow_RsListaRequerimientos = $pageNum_RsListaRequerimientos * $maxRows_RsListaRequerimientos;
	$query_RsListaRequerimientos="SELECT PROVCODI CODIGO_UNI_PROVEEDOR,
										 ...
										 '' FECHA_FIN_CONVENIO,										 
										 ORNCCONS CODIGO_ORDEN_CONVENIO,
										 ORNCFEEN ORDEN_FECHA_ENTREGA,
										 ORNCFECH FECHA_ORDEN,
										 ORNCFIRM FIRMA,
										 IFNULL(ORNCTOEN,'0') TODO_RECIBIDO 
								FROM proveedores_nocotiza,  
									 orden_compra_ncotiza
							WHERE PROVCODI=ORNCIDPN 
...
<div id="pagina">
 ...
 <div class="contenttable">
 	 <input  type="button" class="button2" name="consultar" id="consultar" value="Nueva Compra Directa" onclick="location.href = 'home.php?page=crear_nocotiza';">
...
<?php
	...
	<!--
		...
		<?php if($_SESSION['MM_RolID']==3 && $row_RsListaRequerimientos['FIRMA']=='0'){ ?>
		
		    <a href="O.php?codprov=<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']);?>&codcomp=<?php echo($row_RsListaRequerimientos['CODIGO']);?>&%=0"  class=""  target="_blank" ><img  width="15"  height="20" src='imagenes/compare.png' border='0'></a>
			<input type="button" id="btnfirma_<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>" class="buttonazul" value="Firmar" onclick="Firma_director_admin('<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']); ?>','<?php echo($row_RsListaRequerimientos['CODIGO']);?>','<?php echo($_SESSION['MM_UserID']);?>');"/>
		
		<?php }else{ ?>	
		
			<a href="O.php?codprov=<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']);?>&codcomp=<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>&%=2&f=<?php echo($row_RsListaRequerimientos['FIRMA']);?>"  class=""  target="_blank" ><img  width="15"  height="20" src='imagenes/compare.png' border='0'></a>
		
		<?php } ?> -->
		
		<a href="N.php?codprov=<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']);?>&codcomp=<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>&%=0"  class=""  target="_blank" ><img  width="15"  height="20" src='imagenes/compare.png' border='0'></a>
		...
		if($row_RsListaRequerimientos['TODO_RECIBIDO'] == 0){ ?>
		...
		<input title="Enviar Correo" type="button" id="btnenviarcorr_<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>"  
		class="" value="Enviar correo" onclick="f_EnviarCorreoProv('<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']); ?>','<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>');"/>		
		<?php 
		  }else{
			  ?>
		<input title="Enviar Correo" type="button" id="btnenviarcorr_<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>"  
		class="buttonazul" value="Enviar correo" onclick="f_EnviarCorreoProv('<?php echo($row_RsListaRequerimientos['CODIGO_UNI_PROVEEDOR']); ?>','<?php echo($row_RsListaRequerimientos['CODIGO_ORDEN_CONVENIO']);?>');"/>
...
function f_Enviar_correo(prov,cod)
{
	if(confirm('Esta seguro de enviar este correo?')){	
		    $.ajax({
	            type: "POST",
	            url: "tipo_guardar.php?tipoGuardar=Enviar_correo_Orden_conv&prov="+prov+"&cod_orden_conv="+cod,
	            dataType: 'json',
				success : function(r){					
					if(r>0){
...
function descargar_archivo(prov, cod){
	if(confirm("seguro que desea enviar un correo con el contenido de esta orden no cotiza?")){
	$.ajax({ 
	            type: "POST",
	            url: "N.php?codprov="+prov+"&codcomp="+cod+"&%=1",
	            dataType: 'json',
				success : function(r){		
			
							var nombre_archivo="MC-2015-"+cod;
							//alert('se esta firmado su documento');
						 f_EnviarCorreoProv(prov, cod);
						//alert("se ha registrado correctamente esta orden de compra");
...
function f_EnviarCorreoProv(prov, orden){
	if(confirm("seguro que desea enviar un correo con el contenido de esta orden no cotiza?")){
		$.ajax({
		type: "POST",
		url: "tipo_guardar.php?tipoGuardar=correoordennocotiza&orden="+orden+"&proveedor="+prov,
		dataType: 'json',
		success : function(r){	
			if(r==1){
...
function f_RecibiraProveedor( prov, orden ){
	...
	$('html, body').animate({ scrollTop: $('#popup_cot').offset().top }, 'slow');
			    $.ajax({
	            type: "POST",
	            url: "tipo_guardar.php?tipoGuardar=cargar_orden_nocotiza&orden="+orden,
	            dataType: 'json',
				success : function(r){	
				console.log("antes");
                console.log(r);	
...
var newArrayD = new Array();	
	...
	if(newArrayD.length>0){
		if(confirm("esta seguro de agregar estos productos al proveedor")){
			$.ajax({
	            type: "POST",
	            url: "tipo_guardar.php?tipoGuardar=RecibirProductosNocotiza&orden="+orden+"&sinrecibir="+$("#sinrecibir").val(),
	            dataType: 'json',
				success : function(r){
					//console.log(r);
					if(r.length>0){
