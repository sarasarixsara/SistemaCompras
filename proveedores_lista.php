...
if ($codigo_requerimiento != '') {
	$query_RsListadoDeta_Requ = "SELECT
										...
										DEREJUST AS JUSTIFICACION,
										DEREOBSE AS OBSERVACION,
										DERETISE AS TIPO
									FROM detalle_requ
...
if (!empty($_SERVER['QUERY_STRING'])) {
	...
	foreach ($params as $param) {
		if (
			stristr($param, "pageNum_RsListaProveedores") == false &&
	...
	<div id="pagina">
		<form name="form1" id="form1" action="" method="post">
					...
					<tr>
						...
						<td>
							<input type="text" name="nombre_filtro" id="nombre_filtro"
								value="<?php echo ($nombre_filtro); ?>" class="form-control">
						</td>
						<td>
							<button type="button" class="btn btn-default " name="fcategoria" id="fcategoria"
								onclick="limpiarfiltros('nombre_filtro');"><i class="fa fa-close"></i></button>
						</td>
						<td class="SB">Correo</td>
						<td>
							<input type="text" class="form-control" value="<?php echo ($correo_filtro); ?>"
								id="correo_filtro" name="correo_filtro">
						</td>
					</tr>
					<tr>
						...
						<td>
							<select name="con_conveniofiltro" id="con_conveniofiltro" class="form-control">
								...
								<option value="1" <?php if ($con_conveniofiltro == 1)
									echo ("selected"); ?>>Con Listado
								</option>
								<option value="0" <?php if ($con_conveniofiltro == 0 && $con_conveniofiltro != '')
									echo ("selected"); ?>>Sin Listado</option>
							</select>
						</td>
						<td>
							<button type="button" class="btn btn-default " name="fcategoria" id="fcategoria"
								onclick="limpiarfiltros('con_conveniofiltro');"><i class="fa fa-close"></i></button>
						</td>
						<td class="SB">Categorias</td>
						<td>
							<select name="categoria_prov" id="categoria_prov" class="inputtext form-control"
								style="margin-left:0">
						...
						<td>
							<button type="button" class="btn btn-default " name="fcategoria" id="fcategoria"
								onclick="limpiarfiltros('categoria_prov');"><i class="fa fa-close"></i></button>
						</td>
					</tr>
					<tr>
						<td colspan="6" align="left">
							<button type="submit" name="butonfiltro" id="butonfiltro" class="button2" value="Buscar"><i
			...
			<table class="tablalistado table" cellspacing="2" border="0">
				<tr>
					<td colspan="12">
						<button type="button" name="consultar" id="consultar" value="consultar" class="button2"
							onclick="mostrarfiltros();"><i class="fa fa-filter"></i> Consultar</button>
						<button type="button"
							onclick="location.href='home.php?page=crear_proveedor&tipoGuardar=Guardar';"
							name="crearnuevo" class="button2"><i class="fa fa-plus-circle"></i> Proveedor</button>
						<?php
						if ($_SESSION['MM_RolID'] == 3 || $_SESSION['MM_RolID'] == 2) {
							?>
							<button type="button" onclick="location.href='home.php?page=proveedores_solicitados';"
								name="crearnuevo" class="button2"><i class="fa fa-eye"></i> Proveedores solicitados</button>
							<button type="button" onclick="location.href='home.php?page=proveedores_inconsistencias';"
								name="crearnuevo" class="button2"><i class="fa fa-eye"></i> Proveedores
								...
								<div style="width:30px " class="toolstable">
									...
									<div class="btn-group">
										...
										<ul class="dropdown-menu" role="menu" style="min-width:104px;">
											<li><a class="fz-1" title="Editar"
													onclick="Feditarprov('<?php echo ($row_RsListaProveedores['CODIGO']); ?>');"><i
														class="fa fa-pencil"></i> Editar</a></li>
											<li><a title="Eliminar"
													onclick="Feliminarprov('<?php echo ($row_RsListaProveedores['CODIGO']); ?>');"><i
					...
					<td>
						<ul>
							<?php if ($pageNum_RsListaProveedores > 0) { // Show if not first page ?>
								<li>
									<a href="javascript:f_abrir_link('<?php printf("%s?pageNum_RsListaProveedores=%d%s", $currentPage, 0, $queryString_RsListaProveedores); ?>')"
										class="submenus">Primero</a>
								</li>
							<?php } // Show if not first page ?>
							<?php if ($pageNum_RsListaProveedores > 0) { // Show if not first page ?>
								<li>
									<a href="javascript:f_abrir_link('<?php printf("%s?pageNum_RsListaProveedores=%d%s", $currentPage, max(0, $pageNum_RsListaProveedores - 1), $queryString_RsListaProveedores); ?>')"
										class="submenus">Anterior</a>
								</li>
							<?php } // Show if not first page ?>
							<?php if ($pageNum_RsListaProveedores < $totalPages_RsListaProveedores) { // Show if not last page ?>
								<li>
									<a href="javascript:f_abrir_link('<?php printf("%s?pageNum_RsListaProveedores=%d%s", $currentPage, min($totalPages_RsListaProveedores, $pageNum_RsListaProveedores + 1), $queryString_RsListaProveedores); ?>')"
										class="submenus">Siguiente</a>
								</li>
							<?php } // Show if not last page ?>
							<?php if ($pageNum_RsListaProveedores < $totalPages_RsListaProveedores) { // Show if not last page ?>
								<li>
									<a href="javascript:f_abrir_link('<?php printf("%s?pageNum_RsListaProveedores=%d%s", $currentPage, $totalPages_RsListaProveedores, $queryString_RsListaProveedores); ?>')"
