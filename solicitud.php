...
if ($codigo_requerimiento != '') {
	...
	$query_RsListadoDeta_Requ = "SELECT DR.DERECONS AS CODIGO,
										...
										DR.DERECLAS AS CLASIFICACION,
										TC.TIPONOMB AS CLASIFICACION_DES,
										DR.DEREPRES AS PRESUPUESTO,
										DR.DEREDESC AS DESCRIPCION,
										DR.DERECANT AS CANTIDAD,
										DR.DEREJUST AS JUSTIFICACION,
										DR.DEREOBSE AS OBSERVACION,
										DR.DERECONV AS CONVENI,
										DR.DERETISE AS TIPO,
										DR.DEREVIAT AS CAMPO_VIATICOS,
										DR.DEREELEC AS CAMPO_ELECTRONICA,
										TOC.TOCONOMB AS TIPOORDENCOMPRA_DES,
										CASE DR.DERETISE
											WHEN 0 THEN 'NO SELECCIONADO'
											WHEN 1 THEN 'SELECCIONADO'
											ELSE ''
										END AS TIPO_DES,
										DR.DEREAPRO AS APROBADO,
										ED.ESDENOMB AS APROBADO_DES,
										(SELECT COUNT(D.OBDECODI)
										FROM OBSERVACIONESDET D
										WHERE D.OBDECODE = DR.DERECONS) AS CANTIDAD_OBS,
										P1.PROVNOMB AS PROVEEDOR_DESC_DET,
										P2.PROVNOMB AS PROVEEDOR_RECOT_DESC,
										DR.DERECOSU AS CONSEJO_SUPERIOR,
										CASE DR.DERECOSU
											WHEN 0 THEN ''
											WHEN 1 THEN 'Consejo Superior'
											ELSE ''
										END AS CONSEJO_SUPERIOR_DES,
										DR.DERECOTE AS CONSEJO_TECNOLOGICO,
										CASE DR.DERECOTE
											WHEN 0 THEN ''
											WHEN 1 THEN 'Comite Tecnologico'
											ELSE ''
										END AS CONSEJO_TECNOLOGICO_DES,
										DR.DERECOIN AS COMITE_INFRAESTRUCTURA,
										CASE DR.DERECOIN
											WHEN 0 THEN ''
											WHEN 1 THEN 'Comite Infraestructura'
											ELSE ''
										END AS COMITE_INFRAESTRUCTURA_DES,
										DR.DEREPOA AS POA_DETALLE,
										A.POANOMB AS POA_DETALLE_DES,
										DR.DERENCOT AS DETA_NO_COTIZA,
										UM.UNMESIGL AS UNIDAD_MEDIDA_DES,
										DR.DERESUPO AS SUBPOA_DETALLE,
										PD.PODENOMB AS SUBPOA_DETALLE_DES,
										DR.DEREOTRO AS OTRO_DES,
										DR.DERECOOC AS ORDEN_COTIZADA,
										DR.DEREIOCC AS ORDEN_CONVENIO,
										DR.DERECOMC AS ORDEN_MENOR_CUANTIA,
										DR.DEREREOT AS REQUIERE_OTRO,
										DR.DEREDCOM AS OBSERVACION_PROV,
					
										IFNULL(DR.DEREFERE, '-1') AS FECH_RECIB_USUGENE,
										(SELECT R.REQUENCU FROM requerimientos R WHERE R.REQUCODI = '" . $codigo_requerimiento . "') AS ENCUESTA

									FROM
										DETALLE_REQU DR
									
									LEFT JOIN
										tipo_compra TC ON TC.TICOCODI = DR.DERECLAS
									LEFT JOIN
										tipoorden_compra TOC ON TOC.TOCOCODI = DR.DERETIPO
									LEFT JOIN
										estado_detalle ED ON ED.ESDECODI = DR.DEREAPRO AND ED.ESDEFLAG = 1
									LEFT JOIN
										proveedores P1 ON P1.PROVCODI = DR.DEREPROV
									LEFT JOIN
										proveedores P2 ON P2.PROVCODI = DR.DEREPROV2
									LEFT JOIN
										POA A ON A.POACODI = DR.DEREPOA
									LEFT JOIN
										UNIDAD_MEDIDA UM ON UM.UNMECONS = DR.DEREUNME
									LEFT JOIN
										POADETA PD ON PD.PODECODI = DR.DERESUPO
								
									
									WHERE
										DR.DEREREQU = '" . $codigo_requerimiento . "'";
	...
	if ($totalRows_RsListadoDeta_Requ > 0) {
		...
		do {
			...
			$arr[] = array(
				'ID' => $row_RsListadoDeta_Requ2->CODIGO,
...
<script type="text/javascript">
	...
	function loadclasificacion(id) {
		var code = $("#modalidadcreate_" + id).val();
		$.get("scripts/dependencia-clasificacion.php", { code: code }, function (resultado) {
			if (resultado == false) { alert("Error"); } else {
				$("#clasificacioncreate_" + id).attr("disabled", false); document.getElementById("clasificacioncreate_" + id).options.length = 1; $('#clasificacioncreate_' + id).append(resultado);
			}
		});
	}
...
if (isset($_GET['env']) && $_GET['env'] == '1') {
	...
	<br><br><br>
	<input type="button" name="nbt1" id="nbt1" class="button3" value="Nuevo Requerimiento"
		onclick="CrearRequerimiento();">
	<input type="button" name="nbt2" id="nbt2" class="button3" style="width:140px;" value="Salir"
...
<div id="pagina" style="background:#F3F2F2; margin-top: -15px; min-width:1001px; width:100%; padding-bottom:30px;">
	...
	<div id="titulo">
		<table width="1000" border="0" class="menuoptions" style="background:#F5F5F5; min-width: 1000px; width: 100%;">
			...
			<td align="left" width="110" style="color:#CB5100;"><span
					id="menuinfo">Informaci&oacute;n&nbsp;&nbsp;&nbsp;</span></td>
		</table>
		<div id="content_gbsfw" style="width:500px;  position:absolute; min-height:300px; left:80px;  z-index:-1">
			<div id="gbsfw" class="gb_z fc" guidedhelpid="gbifp"
				style="width: 480px; min-height: 265px;  min-width: 480px; ">
				<div class="fc">
	...
	<?php if ($cedula_sol == $_SESSION['MM_UserID'] and $bandera == 1) { ?>
		<div class="row" style="margin-top:1.5em;">
			<div class="col-md-4 col-xs-5" style="padding-left: 3.2em;">
				<div class="marcar_evento">Evento <input type="checkbox" name="is_evento" id="is_evento" value="1"
						onclick="add_codevento(this)" <?php if ($codigo_evento != '') {
							echo ('checked');
						} ?>> <span
						id="msgaddevento" class="msgaddevento"><?php echo ($codigo_evento); ?></span></div>
				<div class="panel panel-danger" style=" display:<?php if ($codigo_evento != '') {
					echo ('block');
				} else {
					echo ('none');
				} ?>" id="add_codevento">
					...
					<div class="panel-body">
						<div style="display:flex"><input type="text" name="codigo_evento" id="codigo_evento"
								value="<?php echo ($codigo_evento); ?>" class="form-control" style="width:70%">
							<input type="button" class="btn btn-danger" id="" value=" Guardar " style="margin-left:3px"
								onclick="return SaveCodeEvento(); ">
						</div>
					</div>
				</div>
			</div>
			...
			<form action="" id="no_selecc" name="no_selecc" method="post">
				<table id="tabla_noseleccionados" width="400">
					...
					<tr>
						...
						<td>
							<select name="unidad_ns" id="unidad_ns" class="styled-select">
								...
								foreach ($estados as $indice => $registro) {
									?>
									<option value="<?php echo ($registro['CODIGO']) ?>">
					...
					<tr>
						...
						<td><textarea type="text" name="descrip_ns" id="descrip_ns" value="" rows="1" cols="40"
								class=""></textarea></td>
					</tr>
					<tr>
						<td class="SLAB2">Justificacion:</td>
						<td><textarea type="text" name="justi_ns" id="justi_ns" rows="2" cols="40" value=""
								class=""></textarea></td>
					</tr>
					<tr>
						<td class="SLAB2">Observaciones:</td>
						<td><textarea type="text" name="observ_ns" id="observ_ns" rows="2" cols="40" value=""
								class=""></textarea></td>
					</tr>
					<tr>
						<td colspan="6" align="center">
							<div id="botonessub">
								...
								?>
								<input type="submit" class="button2" id="btnsub_ns" value="<?php echo ($infoboton); ?>"
									onclick="return validarCampos('1'); " />
								<input type="button" class="button2" name="limpiar_ns" value="Limpiar"
									onclick="limpiar('0');">
								<?php
	...
	<form name="form_lista" id="form_lista" action="" method="post" enctype="multipart/form-data">
		...
		<?php if ($_SESSION['MM_RolID'] == 3 && ($estado > 2)) {
			...
			<select name="pasar_estado" id="pasar_estado" class="selectgris">
				...
				for ($i = 0; $i < count($arrayestadosDiradm); $i++) {
					?>
					<option value="<?php echo ($arrayestadosDiradm[$i]['CODIGO']); ?>">
						<?php echo ($arrayestadosDiradm[$i]['NOMBRE']); ?>
					</option>
					<?php
				}
				?>
		...
		if ($_SESSION['MM_RolID'] == 2 && ($estado > 2)) {
			...
			<select name="pasar_estado" id="pasar_estado" class="selectgris">
				...
				for ($i = 0; $i < count($arrayestados); $i++) {
					?>
					<option value="<?php echo ($arrayestados[$i]['CODIGO']); ?>"><?php echo ($arrayestados[$i]['NOMBRE']); ?>
					</option>
					<?php
				}
				?>
		...
		?>
		<!--Fin -->
		<table class="bordered" id="tabladetalle"
			style="clear:both; padding-bottom:20px; margin-top:30px; min-width:950px; width:100%">
			<thead>
				<tr>
					<th colspan="3">
						<div id="footer"><span>Listado detalle</span>
						</div>
					</th>
					<th colspan="16">
						...
						<div id="containerinfoeventolista" style="background:#ffffff; margin-left:3px; float:right; padding:3px; border-radius:5px; <?php if ($codigo_evento != '') {
							echo ('display:block');
						} else {
							echo ('display:none;');
						} ?>">
							Tipo de Requerimiento Evento:<br>
							codigo: <span id="msgaddeventolistado"
								class="msgaddevento"><?php echo ($codigo_evento); ?></span>
						</div>
						<div id="multiplesdetalles" style="display: none;margin-left: 10px;float: left;"><button
								type="button" class="btn btn-default">Cargar un archivo a multiples detalles</button>
						</div>
						<button id="markAllReceived" style="padding: 8px;margin-left:10px;"> <img
								src="imagenes/entregado.png" width="16" title="Entregado">Recibido</button>
					...
					<?php if ($_SESSION['MM_RolID'] == 2) {
						...
						<div class="btn btn-xs btn-default"
							style="width:50px; background-color: <?php echo ($colorbandera); ?>;  float:left;"
							onclick="FMR_FbanderaEditar('<?php echo ($codigo_requerimiento); ?>');"><span><img
									src="imagenes/b_edit.png" title="Corregir" width="16"></span></div>
					<?php } ?>
				</tr>
				...
				do {
					...
					<tr class="<?php echo ($estilo); ?>" id="trtr_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>">
						<td>
							...
							<div><input type="checkbox" class="multiplefilechk"
									value="<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
									name="multiplefile_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
									id="multiplefile_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"></div>

							<?php if ($_SESSION['MM_RolID'] == 3) { ?>
								<div>
									<?php if ($row_RsListadoDeta_Requ['APROBADO'] == '6' or $row_RsListadoDeta_Requ['APROBADO'] == '17') { ?>

										<!--Inicio Boton imagen firma director Administrativo -->
										<a id="afirmarDirAdm_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											href="javascript: FirmaDirAdministrativo('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');"><span><i
													class="fa fa-thumbs-up fa-X3" aria-hidden="true"></i></span></a>
										<!--Fin -->
								...
								<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
									<li>
											...
											<?php if ($_SESSION['MM_RolID'] == 2) { ?>
												...
												<?php if ($cedula_sol == $_SESSION['MM_UserID']) { ?>
													...
													<?php if ($row_RsListadoDeta_Requ['APROBADO'] == '2') { ?>
														<div
															onclick="FMD_FcorregirDet('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
															<span><img src="imagenes/save.png" title="Corregir" width="16"></span>
														</div>
													<?php } ?>
													<!--Fin -->
													...
													<div
														onclick="FMD_FeditarDet('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
														<span><img src="imagenes/b_edit.png" title="editar" width="16"></span>
													</div>
													<!--Fin -->
													...
													<div
														onclick="FMD_FdeleteDet('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
														<span><img src="imagenes/delete.jpg" title="Eliminar" width="16"></span>
													</div>
													<?php } ?>
													<!--Fin -->
													...
													<?php if ($row_RsListadoDeta_Requ['APROBADO'] != '8') { ?>
														...
														<div id="recibeusug_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
															onclick="frecibo_usuariog('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
															<img src="imagenes/ent_profe.jpg" title="Recibir" width="16">&nbsp;
														</div>
														<!--Fin -->
													<?php } ?>
												<?php } ?>
												...
												<div
													onclick="mostrarTickbox('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
													<span><img src="imagenes/retroceder.png" width="16" title="Devolver"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen compra electronica un detalle -->
												<div
													onclick="FMD_Electronica('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','<?php echo ($row_RsListadoDeta_Requ['CAMPO_ELECTRONICA']); ?>','<?php echo ($_SESSION['MM_RolID']); ?>');">
													<span><img src="imagenes/tarjetas.png" title="compra electronica"
															width="16"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen viaticos un detalle -->
												<div
													onclick="FMD_Viaticos('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','<?php echo ($row_RsListadoDeta_Requ['CAMPO_VIATICOS']); ?>','<?php echo ($_SESSION['MM_RolID']); ?>');">
													<span><img src="imagenes/avion.jpg" title="viaticos" width="16"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen actualizar pagina -->
												<div onclick="location.reload();"><span><img src="imagenes/devolver.png" width="16"
															title="actualizar"></span></div>
												<!--Fin -->
												<!--Inicio Boton imagen Historial -->
												<div onclick="ver_historial('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
													<span><img src="imagenes/historial.png" width="16" title="historial"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen reiniciar a 0 -->
												<div target="_blank"
													onclick="FMD_ReiniciarDet('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
													<span><img src="imagenes/reiniciar.jpg" width="16" title="Reiniciar a 0"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen comparativo -->
												<div target="_blank"
													onclick="location.href=('comparar.php?tipocompara=detalle&codDetalle=<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>')">
													<span><img src="imagenes/compare.png" width="16"
															title="Comparar Detalle Productos"></span>
												</div>
												<!--Fin -->
												<!--Inicio Boton imagen entregado a usuario general -->
												<div onclick="fentrega('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','0');">
													<span><img src="imagenes/entregado.png" width="16" title="Entregado"></span>
												</div>
												<!--Fin -->
											<?php } ?>
									 ...
									 Fin -->
												...
												<div
													onclick="CancelarDetalle('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
													<span><img src="imagenes/cancelar.png" width="16"
															title="Cancelar - rechazar"></span>
												</div>
												<!--Fin -->
												<!--Fin -->
												<?php if ($row_RsListadoDeta_Requ['APROBADO'] == '32') { ?>
													...
													<div
														onclick="FMD_Menorcuantia('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">
														<span><img src="imagenes/pesos.png" title="menor cuantia" width="16"></span>
													</div>
													<!--Fin -->
												<?php } ?>
									 ...
									 <a id="afirmarRect_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" href="javascript: FirmaRector('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');">Aprobar Compra </a>
									  <!--Fin -->
											...
											<?php if ($_SESSION['MM_RolID'] == 6) { ?>
												...
												<div><a title="aprobar Coordinador"
														onclick="AprobarCoordinador('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');"><i
															class="fa fa-thumbs-up fa-2x" aria-hidden="true"></i><a></div>
												<!--Fin -->
											<?php } ?>
										</div>
									</li>
							...
							<?php if ($_SESSION['MM_RolID'] != 4) { ?>
								<?php if ($_SESSION['MM_RolID'] == 2) { ?>
									...
									<select name="TipoCompra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
										id="TipoCompra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" class="chzn-select"
										...
										foreach ($estados as $indice => $registro) {
											?>
											<option value="<?php echo ($registro['CODIGO']) ?>"><?php echo ($registro['DESCRIPCION']); ?>
											</option>
											<?php
										}
										?>
									...
									<select name="TipoClasCompra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
										id="TipoClasCompra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" class="chzn-select"
									...
									<select name="Convdetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
										id="Convdetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" class="chzn-select"
										onchange="ShowProductosConvenio('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>', this.value)">
										...
										foreach ($estados as $indice => $registro) {
											?>
											<option value="<?php echo ($registro['ID']) ?>"><?php echo ($registro['PROVEEDOR_DES']); ?>
											</option>
											<?php
										}
										?>
									...
									<select id="productoconvenio_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
										name="productoconvenio_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
										class="combo_convenio"
										onchange="FMD_ConvenioProducto('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>',this.value)">
										>
								...
								if ($_SESSION['MM_RolID'] == 3) { ?>
									...
									<div id="divpoadeta_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" style="<?php if ($row_RsListadoDeta_Requ['REQUIERE_OTRO'] == '1') {
										   echo ('display:none;');
									   } ?>">
										<select name="poadetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											id="poadetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" class="chzn-select"
											...
											foreach ($estados as $indice => $registro) {
												?>
												<option value="<?php echo ($registro['POACODI']) ?>"><?php echo ($registro['POANOMB']); ?>
												</option>
												<?php
											}
											?>
										...
										<select name="subpoadetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											id="subpoadetalle_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" class="chzn-select"
											...
											foreach ($estados as $indice => $registro) {
												?>
												<option value="<?php echo ($registro['PODECODI']) ?>"><?php echo ($registro['PODENOMB']); ?>
												</option>
												<?php
											}
											?>
									...
									<input type="checkbox" onclick="Otropoa('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>')"
										name="otro_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" value="1"
										id="otro_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" <?php if ($row_RsListadoDeta_Requ['REQUIERE_OTRO'] == '1') {
											   echo ('checked');
										   } ?>>

									<div id="divotropoa_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>" style="<?php if ($row_RsListadoDeta_Requ['REQUIERE_OTRO'] == '1') {
										   echo ('display:block;');
									   } else {
										   echo ('display:none');
									   } ?>">
										<span class="otropoa">otro</span>&nbsp;
										<input onchange="FMD_OtroPoa('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>')" type="text"
									...
									<div class="actiondetalle">
										<input type="checkbox" id="consejosuperior_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											name="consejosuperior_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											title="Consejo Superior" value="1" <?php if ($row_RsListadoDeta_Requ['CONSEJO_SUPERIOR'] == 1) {
												echo ('checked');
											} ?>>Consejo Superior<br>
										<input type="checkbox"
											id="consejotecnologico_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											name="consejotecnologico_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											title="Comite Tecnologico" value="1" <?php if ($row_RsListadoDeta_Requ['CONSEJO_TECNOLOGICO'] == 1) {
												echo ('checked');
											} ?>>Comite
										Tecnologico<br>
										<input type="checkbox" id="comiteinfra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											name="comiteinfra_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
											title="Comite Infraestructura" value="1" <?php if ($row_RsListadoDeta_Requ['COMITE_INFRAESTRUCTURA'] == 1) {
												echo ('checked');
											} ?>>Comite
										Infraestructura<br>
									</div>
						...
						<td>
							...
							//mensajes concejos
							if ($row_RsListadoDeta_Requ['CONSEJO_SUPERIOR'] == 1) {
								echo ('<div class="infodetest" >*Consejo Superior</div><br>');
							}
							if ($row_RsListadoDeta_Requ['CONSEJO_TECNOLOGICO'] == 1) {
								echo ('<div class="infodetest" >*Consejor Tecnologico</div><br>');
							}
							if ($row_RsListadoDeta_Requ['COMITE_INFRAESTRUCTURA'] == 1) {
								echo ('<div class="infodetest" >*Comite Infraestructura</div><br>');
							}

							?>
						</td>
						<td title="<?php echo ($row_RsListadoDeta_Requ['APROBADO_DES']); ?>"
							bgcolor="<?php echo ($claseaprobado); ?>">
							<?php echo ($i); ?>
						</td>
						<td>
							<p id="parrafodescr_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>">
								<?php if (strlen($row_RsListadoDeta_Requ['DESCRIPCION']) < 100) {
									echo ($row_RsListadoDeta_Requ['DESCRIPCION']);
								} else {
									echo (substr($row_RsListadoDeta_Requ['DESCRIPCION'], 0, 100)); ?>&nbsp;&nbsp;<a
										href="javascript:crearfila('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','desc');">...</a><?php } ?><br>
							</p>
						</td>
						<td>
							<?php if ($_SESSION['MM_RolID'] != 4) { ?>
								<input type="text"
									onChange="FMD_CambiarCantidad('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');"
									name="inpcantidad_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
									id="inpcantidad_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
									value="<?php echo ($row_RsListadoDeta_Requ['CANTIDAD']); ?>" size="4">
							<?php } else { ?>
						...
						<td>
							<p><?php if (strlen($row_RsListadoDeta_Requ['JUSTIFICACION']) < 100) {
								echo ($row_RsListadoDeta_Requ['JUSTIFICACION']);
							} else {
								echo (substr($row_RsListadoDeta_Requ['JUSTIFICACION'], 0, 100)); ?>&nbsp;&nbsp;&nbsp;<a
										href="javascript:crearfila('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','just');">...
									</a><?php } ?><br></p>
						</td>
						<td>
							<p><?php if (strlen($row_RsListadoDeta_Requ['OBSERVACION']) < 100) {
								echo ($row_RsListadoDeta_Requ['OBSERVACION']);
							} else {
								echo (substr($row_RsListadoDeta_Requ['OBSERVACION'], 0, 100)); ?>&nbsp;&nbsp;&nbsp;<a
										href="javascript:crearfila('<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>','obs');">...
									</a><?php } ?><br></p>
						</td>
						<td>
							<?php
						...
						<td>
							<?php if ($_SESSION['MM_RolID'] == 3) { ?>
								<input onChange="FMD_validarSiNumero(this.value);" type="text"
									name="presup_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
									id="presup_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"
						...
						<td>
							<?php if ($_SESSION['MM_RolID'] != 4) { ?>
								<div class="FactDetalles" id="commentdet_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"><span
										class="numcoment"></span></div>
							<?php } else { ?>
								<?php echo (''); ?>
							<?php } ?>
						</td>
						<td>
						 ...
						 ";
								...
								<?php if ($row_RsListadoDeta_Requ['ORDEN_COTIZADA'] != '0') { ?>
									<div><a target="_blank"
											href="http://compras.sanboni.edu.co/O.php?codprov=<?php echo ($row_RsConsultaFirma_orden['ORDEN_PROVEEDOR']); ?>&codcomp=<?php echo ($row_RsConsultaFirma_orden['ORDEN_COD']); ?>&%=2&f=<?php echo ($row_RsConsultaFirma_orden['FIRMA_ID']); ?>"><?php echo ($row_RsConsultaFirma_orden['ORDEN_COD']); ?>

										</a></div>

									<?php if ($_SESSION['MM_RolID'] == 3 && $totalRows_RsConsultaOrdenAutorizaAdm > '0') { ?>

										<input type="button" id="btnautoriza_<?php echo ($row_RsConsultaFirma_orden['ORDEN_COD']); ?>"
											class="buttonazul" value="Autorizar"
											onclick="visto_director_admin('<?php echo ($row_RsConsultaFirma_orden['ORDEN_COD']); ?>','<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>');" />
								...
								if ($row_RsListadoDeta_Requ['ORDEN_CONVENIO'] != '0') { ?>
									<div><a target="_blank"
											href="http://190.107.23.165/compras/C.php?codprov=<?php echo ($row_RsConsultaOrdenConvenio['CODIGO_UNI_PROVEEDOR']); ?>&codcomp=<?php echo ($row_RsListadoDeta_Requ['ORDEN_CONVENIO']); ?>&%=0"><?php echo ($row_RsListadoDeta_Requ['ORDEN_CONVENIO']); ?></a>
									</div>&nbsp;
									<?php
								}
								...
								if ($row_RsListadoDeta_Requ['ORDEN_MENOR_CUANTIA'] != '0') { ?>
									<div><a target="_blank"
											href="http://190.107.23.165/compras/N.php?codprov=<?php echo ($row_RsConsultaOrdenMenorCuantia['CODIGO_UNI_PROVEEDOR_MC']); ?>&codcomp=<?php echo ($row_RsListadoDeta_Requ['ORDEN_MENOR_CUANTIA']); ?>&%=0"><?php echo ($row_RsListadoDeta_Requ['ORDEN_MENOR_CUANTIA']); ?></a>
									</div>&nbsp;
									<?php
								}
							} ?>
						...
						<td>
							...
							<div class="AnexosDetalles" id="commentdet_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"><span
									class="numcoment"></span></div>

						</td>
						<td>
							<div class="comentdetalle" id="commentdet_<?php echo ($row_RsListadoDeta_Requ['CODIGO']); ?>"><span
									class="numcoment"><?php echo ($row_RsListadoDeta_Requ['CANTIDAD_OBS']); ?></span></div>
						</td>
				...
				<tr bgcolor="#F4F3F1">
					<td colspan="19" align="center">
						<?php if ($_SESSION['MM_RolID'] == 2 && $estado == 2) { ?>
							...
							<input type="button" class="button2" id="btnsub_ns" value="No Recibir"
								onclick="return Recibir('4'); ">
						<?php } ?>
		...
		if ($totalRows_RsArchivosLista) {
			...
			<table style="margin-left:10px;" width="600" BORDER="0">
				...
				do {
					...
					<tr>
						<td height="35"><a
								href="downloadfile.php?doc=<?php echo ($row_RsArchivosLista['ARCHIVO']); ?>&tipopath=ug"
								target="_blank" class="buttonrojo">descargar</a></td>
						<?php if ($_SESSION['MM_RolID'] == 4) { ?>
							<td height="35"><a class="buttonrojo"
									onclick="ElimArchivo('<?php echo ($row_RsArchivosLista['CODIGO']) ?>');">eliminar</a></td>
						<?php } ?>
	...
	<table border="0" width="800">
		<tr>
			<td colspan="6" align="center">
				<input class="button2" type="button" value="Volver a Listado Requerimientos"
					onclick="return volveraListado();" />
			</td>
		</tr>
	</table>
...
<script
	type="text/javascript"> $(".chzn-select, .combo_convenio").chosen(); $(".chzn-select-deselect").chosen({ allow_single_deselect: true }); </script>
