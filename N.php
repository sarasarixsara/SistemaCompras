...
require_once("conexion/db.php");
  $query_RsDetallesCompra ="SELECT OC.ORNCCONS CODIGO_ORDEN_NOCOTIZA,
								   PV.PROVNOMB PROVEEDOR_DES,
								   PV.PROVREGI PROVEEDOR_NIT,
								   PV.PROVTELE PROVEEDOR_TELEFONO,
								   PV.PROVIDCI CIUDAD,
								   PV.PROVDIRE DIRECCION,
								   DR.DERECONS DETALLE_CODIGO,
								   DR.DEREDESC DETALLE_DES,
								   DR.DERECANT  DETALLE_CANTIDAD,
								   OD.OCDNDESC DESC_COMPR,
								   OD.OCDNVAIV  IVA_VALOR,
								   OD.OCDNVAUN VALOR_UNITARIO_CONVENIDO,
								   'MENOR CUANTIA'TIPO_ORDEN_DES,
								   '' PROVEEDOR_CIUDAD,
								   (SELECT R.REQUCORE FROM REQUERIMIENTOS R WHERE R.REQUCODI=DR.DEREREQU) REQUERIMIENTO,
								   OD.OCDNVAUN V_PRODUCTO
								   
								   
							FROM
							orden_compra_ncotiza     OC,
							orden_compradet_nocotiza OD,
							proveedores_nocotiza	   PV,
							detalle_requ             DR
							WHERE OC.ORNCCONS = '".$_GET['codcomp']."'
							  AND OC.ORNCCONS = OD.OCDNORCO
							  AND OC.ORNCIDPN = PV.PROVCODI
							  AND OD.OCDNDETA = DR.DERECONS";
    ...
    public function Header() {
		...
		$tbl='<table border="0" width="100%" border="0">
				 <tr style="line-height:25px;">
				   <td align="center" rowspan="2" width="50%"><FONT SIZE="9">FORMATO ORDEN MENOR CUANTIA '.$this->header_title.'</FONT></td>
...
$pdf->SetTitle('orden de compra menor cuantia');
$pdf->SetSubject('San Bonifacio orden no cotiza');
$pdf->SetKeywords('Orden no cotiza, PDF, Sanbonifacio, Detalle');
...
// -----------------------------------------------------------------------------
		...
		$query_RsOrdenCom=" SELECT `ORNCCONS` CODIGO,
								   `ORNCFECH` FECHA,
								   '' CONVENIO,
								   `ORNCFEEN` FECHA_ENTREGA,
								   `ORNCOBSE` OBSERVACION,
								    ORNCDIVA  D_IVA
						   FROM orden_compra_ncotiza 
						   WHERE `ORNCCONS`='".$_GET['codcomp']."'";
...
$tbl='<table border="0" width="100%">
         <tr align=""><td width="10%"><FONT SIZE="9">Fecha: 		</FONT></td><td width="40%"><FONT SIZE="9">'.$row_RsOrdenCom['FECHA'].'</FONT></td>
		              <td width="10%"><FONT SIZE="9">Numero: 		</FONT></td><td width="40%"><FONT SIZE="9">MC-2015-'.$row_RsOrdenCom['CODIGO'].'</FONT></td>
		 </tr>
...
$array_requ = array();
		...
		if($totalRows_RsDetallesCompra>0){
		 ...
		 do{
			...
			$valortotal=  (($row_RsDetallesCompra['VALOR_UNITARIO_CONVENIDO'] * $row_RsDetallesCompra['DETALLE_CANTIDAD'])+$row_RsDetallesCompra['IVA_VALOR']);
			$arraytotal[] = $valortotal;
		    $tbldata =$tbldata.'
			 <tr nobr="true" '.$color.'>		   
			   <td align="justify"><FONT SIZE="7"> '.$row_RsDetallesCompra['DESC_COMPR'].'<BR>'.$row_RsDetallesCompra['DETALLE_CODIGO'].'-'.$row_RsDetallesCompra['DETALLE_DES'].'</FONT></td>
...
$pdf->Image('imagenes/firmas/39550544.png', '', '', 70, 30, '', '', 'T', false, 200, '', false, false, 0, false, false, false);
...
//Close and output PDF document
	...
	$nombre_archivo='MC-2015-'.$codigo_orden;
