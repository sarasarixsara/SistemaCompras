  ...
  if($("#provasigdet_marcado").val() != '' ){
	 if($("#provasigdet").val() == $("#provasigdet_marcado").val()){
		 return;
   	    ...
   	    $.ajax({
	            type: "POST",
	            url: "tipo_guardar.php?tipoGuardar=ConsultarDataOrden&provasigdet="+$("#provasigdet").val(),
	            dataType: 'json',
				success : function(r){
				      ShowData(r);
					  ordendet = r;
...
tipordn = $('#tipoorden_compra').val();
	if(confirm('Esta seguro de ordenar esta compra?')){	
		    $.ajax({
	            type: "POST",
	            url: "tipo_guardar.php?tipoGuardar=Ordenarcompra&provasigdet="+$("#provasigdet").val()+"&fecha_entrega="+$("#fecha_entrega").val()+"&observacion="+$("#observacion").val()+"&iva_desc="+$("#iva_desc").val()+"&tipo_orden="+tipordn,
	            dataType: 'json',
				success : function(r){
					if(r>0){
