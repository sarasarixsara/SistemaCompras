					...
					if($ultimo_usuario_creado > 0){
						...
						if($totalRows_RsDatosUsuario > 0){
							$query_RsInsertarPersona = "INSERT INTO PERSONAS (
																	...
																	// exit($query_RsInsertarInsertarPersona);
...
if($tipoGuardar == 'FirmarDirectorioAdm'){
	$query_RsUpdateFirma = "UPDATE detalle_requ set DEREAPRO = '17',
	                                                DEREFIRM = '1',
                                                    DEREFFDA = SYSDATE()	
	                         where DERECONS = '".$_GET['codigo_detalle']."'";
    $RsUpdateFirma = mysqli_query($conexion,$query_RsUpdateFirma) or die(mysqli_error($conexion));	
			...
			if($convenio_corr != '' or $compra_normal > 0 ){				
                ...
                if($totalRows_RsCorreo_proveedor>0){
					...
					$tema = $tema.'     	<p>											
											...
											';//el tema del correo
					$link				='http://190.107.23.165/compras/';
					$nombre_link		='RECIBIR';
...
if($tipoGuardar=='Enviar_correo_Orden_conv'){
	...
	$totalRows_Rsproveedor = mysqli_num_rows($Rsproveedor);
	 ...
	 if($totalRows_Rsproveedor>0){
					...
					$imagen_cabecera	='http://190.107.23.165/compras/imagenes/8.1.png';//imagen de cabecera
	 ...
	 if($totalRows_Rsproveedor>0)
	 	{
					...
					$tema               ='Usted a sido seleccionado por la Corporaci&oacute;n  Colegio San Bonifacio, sea  muy amablemente realizar 
										  la entrega de la siguiente orden de '.$orden_desc.' que se encuentra adjunta, muchas gracias por su 
										  participación 
										  <br><br>
										  </p>';			
					$tema = $tema.'       <p>											
											<br><br>
											<font color="#666666">
										   <h2 align="justify">Por seguridad las ordenes generadas por  la Corporaci&oacute;n  
										   			           Colegio San Bonifacio tienen contraseña, se pueden verificar en el portal
										   			           de compras  para abrir el documento.</h2>
							               <br>
										   <h2>Para verificar la firma del documento de clic al siguiente link:</h2>
							               </font>										
											';//el tema del correo
					$link				='http://190.107.23.165/compras/validar_firma.php?key='.$codigo_firma;
