			...
			if ($totalRows_RsListaRequerimientos > 0) {
				...
				<ul class="nav nav-tabs">
					...
					for ($k = 0; $k < count($arraytabs); $k++) {
						$t++;
						?>
						<li><a href="#provcodord_<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>"
								data-toggle="tab"><?php echo ($arraytabs[$k]['PROVEEDOR_DES']); ?></a></li>
						<?php
					}
					?>
				</ul>
							...
							for ($k = 0; $k < count($arraytabs); $k++) {
								...
								<div id="provcodord_<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>" class="tab-pane" style="width:950px;">
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<label
												class="col-md-4 control-label"><?php echo ($CODIGOCOTIZACION . $arraytabs[$k]['COTIZACION']); ?></label>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<label class="col-md-4 control-label"><span
													id="estadocotizacion_<?php echo ($arraytabs[$k]['COTIZACION']); ?>"><?php echo ($arraytabs[$k]['ESTADO_DE']); ?></span></label>
										</div>
									</div>
									<br>
											...
											if ($arraytabs[$k]['PROVEEDOR'] == $arrayreqprov[$j]['PROVEEDOR']) {
												...
												<tr>
													...
													<td>
														...
														?>
														<select class="tipocompra"
															name="tipocompra_<?php echo ($arrayreqprov[$j]['CODIGO_DETALLE']); ?>_<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>"
															id="tipocompra_<?php echo ($arrayreqprov[$j]['CODIGO_DETALLE']); ?>_<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>"
															Onchange="GuardarTipoCompra('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>','<?php echo ($arrayreqprov[$j]['CODIGO_DETALLE']); ?>');">
															<option value="">Seleccione...</option>
															<?php
															if (count($arraytipocompra) > 0) {
																for ($n = 0; $n < count($arraytipocompra); $n++) {
																	?>
																	<option value="<?php echo ($arraytipocompra[$n]['CODIGO']); ?>" <?php if ($arrayreqprov[$j]['TIPOCOMPRA'] == $arraytipocompra[$n]['CODIGO']) {
																		 echo ('selected');
																	 } ?>><?php echo ($arraytipocompra[$n]['NOMBRE']); ?></option>
																	<?php
																}
															}
															?>
													...
													<td><a href="home.php?page=solicitud&codreq=<?php echo ($arrayreqprov[$j]['CODI_REQUERIMIENTO']); ?>"
															class="btn btn-link"
															role="button"><?php echo ($arrayreqprov[$j]['REQUERIMIENTO']); ?></a></td>
												</tr>
									...
									<div class="row">
										<div class="col-md-2" style="width:10%;">
											<input type="button" class="btn btn-sm btn-primary" value="Enviar correo"
												onclick="enviarcorreo('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>
										<div class="col-md-2" style="width:11%;">
											<input type="button" class="btn btn-sm btn-primary" value="Reenviar correo"
												onclick="enviarcorreo('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>
										<div class="col-md-2" style="width:50%;">
											<input type="button" class="btn btn-sm btn-primary" value="Llenar Manual Productos"
												onclick="Vmanual('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>
										<div class="col-md-2" style="width:10%;">
											<input type="button" class="btn btn-sm btn-primary" value="Llenar Manual servicios"
												onclick="Vmanualservi('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>
										<div class="col-md-2" style="width:10%;">
											<input type="button" class="btn btn-sm btn-primary" value="Llenar Manual Trabajos"
												onclick="Vmanualtrab('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>
										<div class="col-md-1" style="width:10%;">
											<input type="button" class="btn btn-sm btn-primary" value="  Listo  "
												onclick="listocotizado('<?php echo ($codorden); ?>','<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>','<?php echo ($arraytabs[$k]['COTIZACION']); ?>');">
										</div>

										<div class="col-md-2" style="width:10%;">
											<input type="button" class="btn btn-sm btn-primary previsualizar" value="Previsualizar"
												data-prov="<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>">
										</div>
										<span id="spanmsgprov_<?php echo ($arraytabs[$k]['PROVEEDOR']); ?>"
											style="color:#4d9911; display:none; background:#fffdc1; padding:5px;">correo enviado</span>
											...
											<div class="modal-dialog">
												...
												<div class="modal-content">
													...
													<div class="modal-body">
															...
															<tr>
																<td width='613'>
																	<table width='100%' border='0'>
																		...
																		<tr>
																			<td colspan='2'>
																				<h2>Reciba un cordial saludo de parte de la
																					Corporaci&oacute;n Colegio San Bonifacio de las
																					lanzas:</h2>
																			</td>
																		</tr>
																		<tr>
																			<td colspan=''>
																				<h2><small>Se&ntilde;or(a)</small></h2>
																			</td>
																		</tr>
																		<tr>
																			<td colspan=''>
																				<h2><small>NOMBRE DEL PROVEEDOR</small></h2>
																			</td>
																		</tr>
																	</table>
																	<br>
																	<br>
																	<p class="text-justify"><strong>Por Favor, se solicita muy
																			amablemente cotizar los siguientes items atravez del PORTAL
																			WEB DE COMPRAS de la Corporaci&oacute;n Colegio San
																			Bonifacio de las lanzas: </strong></p>
																	<br>
																</td>
															</tr>
														</table>
														...
														<p class="text-justify  text-warning"><small>Ante cualquier inquietud, puede
																comunicarse con la encargada del Proceso de
																Contrataci&oacute;n - EDNA CONSUELO CARVAJAL OYUELA, en el teléfono
																2670226 Ext.116 ó 311 538 3277,
																o email edna.carvajal@sanboni.edu.co, quien le brindará su
